...
WEEK_END = "2026-07-19"
WEEK_LABEL = "July 13–19, 2026"
AI_CREDIT = "ChatGPT 5.6 Sol Max and Claude Fable 5 Max (Cowork)"
...
def header(active: str, prefix: str) -> str:
    links = [
        ...
        ("national", "National", f"{prefix}national/index.html"),
        ...
        ("archive", "Full archive", f"{prefix}archive/index.html"),
    ]
    nav = "".join(
...
<header class="site-header">
  <div class="header-inner">
    <a class="brand" href="{prefix}index.html"><span class="brand-mark">TR</span><span><strong>THE RECORD</strong><span>Evidence · context · accountability</span></span></a>
...
def footer(prefix: str) -> str:
  ...
  <section><h2>The Record</h2><p>A sourced accountability archive edited by Phillip Linstrum. Facts, analysis, and responses are visibly separated. Corrections are timestamped; right-of-response material is attached to the relevant record.</p></section>
...
</div><div class="footer-bottom">Release {VERSION} · checked {CHECKED_AT} · built with {AI_CREDIT}, under editor review.</div></footer>
<div class="toast" role="status" aria-live="polite"></div>'''


def document(*, title: str, description: str, body: str, active: str, prefix: str, extra_head: str = "") -> str:
    return f'''<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="{esc(description, quote=True)}"><meta name="theme-color" content="#0b1320">{extra_head}<title>{esc(title)}</title><link rel="stylesheet" href="{prefix}assets/styles.css"></head><body>
...
def weekly_page(entries: list[dict], ledger: dict) -> str:
    ...
    body = f'''<div class="container"><header class="page-head"><div class="eyebrow">What happened this week</div><h1>{WEEK_LABEL}</h1><p>This is a reproducible seven-day view anchored to the July 19 release—not a browser-clock guess. It includes {len(weekly)} records: {national_count} national and {in6_count} IN-6.</p><div class="scope-filter" aria-label="Filter weekly records"><button class="button button--primary" type="button" data-week-filter="all" aria-pressed="true">All {len(weekly)}</button><button class="button button--ghost" type="button" data-week-filter="national" aria-pressed="false">National {national_count}</button><button class="button button--ghost" type="button" data-week-filter="in6" aria-pressed="false">IN-6 {in6_count}</button></div></header>{search_panel("Search this week")}<div class="integrity-note"><h2>Currentness boundary</h2><p>Official White House, Justice Department, House, and IN-6 publication channels were checked through {CHECKED_AT}. The July 16–17 election-security disclosure is the new national record in this pass. Per-entry check times remain visible.</p></div><section class="record-list">{cards}</section></div>'''
...
def downloads_page(entries: list[dict]) -> str:
    featured = [
        ("Complete July 19 update", "All ten entry packs, both briefs, source ledgers, and machine-readable data.", "../artifacts/THE_RECORD_JULY_19_UPDATE_PACK_2026-07-19.zip", ROOT / "artifacts/THE_RECORD_JULY_19_UPDATE_PACK_2026-07-19.zip", "Download complete update"),
        ("National current brief", "Six national entries, including the July 19 election-security addition, with individual packs and the full source ledger.", "../artifacts/THE_RECORD_NATIONAL_UPDATE_PACK_2026-07-19.zip", ROOT / "artifacts/THE_RECORD_NATIONAL_UPDATE_PACK_2026-07-19.zip", "Download national pack"),
        ("IN-6 current brief", "Four IN-6 entries preserved from the July 18 package; today’s official check found no newer qualifying district item.", "../artifacts/THE_RECORD_IN6_UPDATE_PACK_2026-07-18.zip", ROOT / "artifacts/THE_RECORD_IN6_UPDATE_PACK_2026-07-18.zip", "Download IN-6 pack"),
        ("Current-entry data", "All ten dated entries in one JSON file for reuse in future builds or research tools.", "../data/current_entries.json", ROOT / "data/current_entries.json", "Download entry JSON"),
        ("Source ledger CSV", "A flat audit table of all 28 sources linked in this current layer.", "../data/source_ledger.csv", ROOT / "data/source_ledger.csv", "Download source CSV"),
...
<article class="method-card" id="ai-disclosure"><h2>6. AI disclosure &amp; provenance</h2><p>Current research organization, drafting support, code, and adversarial review use <strong>ChatGPT 5.6 Sol Max</strong> and <strong>Claude Fable 5 Max (Cowork)</strong>. Phillip Linstrum remains the editor and acceptance authority. AI output is not a source.</p><p>The five April 2026 AI Opinion essays retain their original attribution to <strong>Claude (Anthropic, Opus 4)</strong>. This July 19 pass updates the surrounding maintenance disclosure; it does not silently reassign or rewrite those essays’ authorship. See <a href="../AI_PROVENANCE.md">the provenance record</a>.</p></article>
</section>
<div class="integrity-note"><h2>Scope of this release</h2><p>This candidate contains ten dated updates and a navigable publication layer. It does not certify the complete legacy archive, independently validate every historical source, or resolve all open IN-6 claims. The open issues list is part of the product.</p></div>
...
def home_page(entries: list[dict], ledger: dict) -> str:
    ...
    body = f'''<section class="hero"><div class="hero-inner"><div><div class="kicker">Daily accountability release · {RELEASE_DATE}</div><h1>The evidence should be easier to reach than the rhetoric.</h1><p>The Record now has a working weekly view, a first-class Agencies route, current AI disclosure, national developments, IN-6 representation, source ledgers, and direct downloads. Every update remains source-bound and challengeable.</p><div class="hero-actions"><a class="button button--primary" href="weekly/index.html">What happened this week</a><a class="button button--secondary" href="agencies/index.html">Browse agencies</a><a class="button button--ghost" href="downloads/index.html">Download the update</a></div></div><aside class="hero-stamp"><div class="eyebrow">Release state</div><strong>Current through 8:15 AM EDT</strong><p>One new national record was added in today’s check. The long-form legacy archives remain separate and preserved.</p></aside></div><figure class="hero-art"><img src="assets/brand/the-record-hero.png" alt="An illuminated evidence archive connecting sourced records across a living accountability timeline" width="1672" height="941" fetchpriority="high" decoding="async"></figure></section>
<div class="container"><section class="stats" aria-label="Release statistics"><div class="stat"><strong>{national_count}</strong><span>national updates</span></div><div class="stat"><strong>{in6_count}</strong><span>IN-6 updates</span></div><div class="stat"><strong>{len(entries)}</strong><span>individual download packs</span></div><div class="stat"><strong>{len(ledger)}</strong><span>source-ledger records</span></div></section>
<section class="weekly-highlight"><div><div class="eyebrow">What happened this week</div><h2>{len(weekly)} records · {WEEK_LABEL}</h2><p>A fixed seven-day window, searchable and filterable by National or IN-6.</p><a class="button button--primary" href="weekly/index.html">Open the weekly record</a></div><ul>{weekly_links}</ul></section>
<div class="integrity-note"><h2>Integrity finding: archive totals need reconciliation</h2><p>The legacy repository currently displays conflicting exact totals in its repository description, README headline, and architecture notes. This release does not repeat an unreconciled number as fact. Until canonical source arrays generate one authoritative count, the public description should say <strong>4,000+ entries and 5,000+ sources</strong>.</p></div>
<div class="section-head"><div><div class="eyebrow">Navigate</div><h2>One clear route for each kind of question</h2></div><p>The current release is a front door and evidence layer. It preserves—not replaces—the complete archive.</p></div><section class="route-grid">
<article class="route-card"><div class="eyebrow">Weekly</div><h3>What happened this week</h3><p>Seven current records in a stable July 13–19 window, with working scope filters and search.</p><a class="button button--ghost" href="weekly/index.html">Open weekly</a></article>
<article class="route-card"><div class="eyebrow">National</div><h3>National current record</h3><p>Six sourced developments, each separated into facts, significance, and the administration’s response.</p><a class="button button--ghost" href="national/index.html">Open national</a></article>
...
<article class="route-card"><div class="eyebrow">Portable</div><h3>Download center</h3><p>Download today’s package, either brief, machine-readable data, checksums, or any single entry.</p><a class="button button--ghost" href="downloads/index.html">Open downloads</a></article>
<article class="route-card"><div class="eyebrow">Sources</div><h3>Source ledger</h3><p>Inspect every primary, official, and independent source in the current layer.</p><a class="button button--ghost" href="sources/index.html">Open source ledger</a></article>
<article class="route-card"><div class="eyebrow">Preserved</div><h3>Complete legacy archives</h3><p>The thousands-entry national archive and full IN-6 audit remain available and are not overwritten.</p><a class="button button--ghost" href="archive/index.html">Open full archives</a></article>
</section></div>'''
    return document(title=f"{RELEASE_DATE} release · The Record", description=f"The Record daily accountability release for {RELEASE_DATE}.", body=body, active="home", prefix="")


def archive_page() -> str:
    body = '''<div class="container"><header class="page-head"><div class="eyebrow">Preserved history</div><h1>Open the complete archives</h1><p>The July 19 candidate is the current front door. The long-form national and IN-6 archives remain available at their established repository paths and public builds.</p></header><section class="download-grid">
<article class="download-card"><div class="eyebrow">National</div><h3>Complete Trump-accountability archive</h3><p>Open the preserved single-file application with the full historical timeline, topic filters, people, AI Opinion essays, methodology, and companion-document references.</p><a class="button button--primary" href="../the-record.html">Open complete archive</a><p class="micro"><a href="https://github.com/PauseBeforeHarmProtocol/the-record" target="_blank" rel="noopener">Source repository</a> · <a href="https://raw.githubusercontent.com/PauseBeforeHarmProtocol/the-record/main/the-record.html" target="_blank" rel="noopener">Raw single-file HTML</a></p></article>
...
<article class="download-card"><div class="eyebrow">Integrity</div><h3>Why this layer does not replace the archive</h3><p>A daily release should be small enough to inspect. The historical applications remain the canonical long-form record until their source, counts, and build pipeline are deliberately migrated.</p><a class="button button--ghost" href="../methodology/index.html">Read migration rules</a></article>
</section><div class="integrity-note"><h2>Preservation rule</h2><p>Keep <code>the-record.html</code>, <code>entries_array.js</code>, companion documents, the complete PDF, and <code>docs/</code>. The current front door routes users into those assets; it does not delete them.</p></div></div>'''
...
def not_found_page() -> str:
    body = '''<div class="container"><header class="page-head"><div class="eyebrow">404</div><h1>That record path does not exist.</h1><p>Use the current directory or open the preserved legacy archive.</p><div class="button-row"><a class="button button--primary" href="index.html">Return home</a><a class="button button--ghost" href="archive/index.html">Open full archive</a></div></header></div>'''
...
def build_pages() -> dict[Path, str]:
    ...
    return {
        ...
        ROOT / "methodology/index.html": methodology_page(),
