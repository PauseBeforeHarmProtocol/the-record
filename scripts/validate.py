...
ALLOWED_MAINTENANCE_KINDS = {
    "current_maybe_therefore_backfill",
    "repository_recovery",
}
...
if 'src="current_layer_bridge.js"' not in legacy_text:
    fail("historical archive does not load current_layer_bridge.js by a base-path-safe relative URL")
if 'src="/the-record/current_layer_bridge.js"' in legacy_text:
    fail("historical archive retains a deployment-root-specific current-layer bridge URL")
if 'id="currentLayerLoadWarning"' not in legacy_text:
    fail("historical archive lacks a visible current-layer load-failure warning")
if "CURRENT_LAYER_BRIDGE_STATUS_START" not in legacy_text or "warning.hidden=false" not in legacy_text:
    fail("historical archive does not activate its current-layer load-failure warning")
...
if 'rel="canonical" href="../the-record.html"' not in docs_text:
    fail("docs compatibility archive does not name the single canonical archive runtime")
if "new URL('../the-record.html',window.location.href)" not in docs_text:
    fail("docs compatibility archive does not redirect interactive visits to the live runtime")
if "target.hash=window.location.hash" not in docs_text:
    fail("docs compatibility redirect does not preserve deep links")
